...
2025年2月26日
完成首页仪表盘的租赁到期检测功能
完成了告警列表的设置和告警列表页面功能雏形

---
2025年1月23日
完成租赁历史记录页面
完成租赁到期检查页面（分5日内到期和10日内到期）

---
2025.01.22
完成了ACL生成历史日志列表
完成了代码在github上的更新
完成了序列号统计及财务销售统计（根据序列号）功能
修复了流量服务器选中以后只能显示该服务器下的容器功能
增加了流量状态页面的服务器和容器的搜索功能
修复了历史流量记录的显示（单位和图表）
---

2025.01.21
优化了首页仪表盘功能
完成了流量保存功能，多个数据库按照容器为逻辑，保存全部流量。
完成了首页的system_overview的API设计，并将逐步优化
优化了容器列表的界面，将ip及运行状态做了两个超链接结构。

---
2025.01.17
完成了用户模块的大多数功能，调试和测试了程序代码；
完成了流量模块的框架，完成了实时监控模块的功能；
还需要进一步完善细节和功能。
---

2025.01.16
完成了租赁代码的列表三个按钮的功能模块，调试了数据表操作的完整性
完成了新购华为云，搭建了内地的程序
完成了流量板块的基础框架搭建
---

2025.01.15
今日完成了租赁模块的大部分功能，优化了UI显示
完成了数据表及后端相关的rental_rooute.py代码的优化
完成了基础的租赁、创建租赁、列表等功能的测试
---

2025.01.14
今日写了租赁相关的前端6个文件以及对话框，优化了api，调整了相关页面

---
2025.01.13
今日完成了数据库重构，主要是对租赁关系中产生的各个数据表之间的复杂关系进行了梳理
完成了服务器列表小bug修复
完成了数据全部清空并重新创建以及数据的测试

---
2025.01.12
完成了生成acl的前端代码和后端代码，已经能成功发送email邮件，并标准化了格式输出
完成了数据库写入和acl_logs数据库写入操作
完成了ACL界面优化布局
...
2025.01.11
完成了仪表盘页面的重构，增加了鼠标划过的呼吸样式
完成了服务器列表页面的分类和状态的修复
完成了服务器列表健康检查的前端和后端代码的重构
完成了容器列表页面的服务器名称和服务器公网IP的获取逻辑（后期考虑前端获取改为后端）
完成了running的超链接直接到derp服务器状态页面
修复了容器列表页面的单个容器修改的功能，但是好像没办法更新数据库，要调试和修复
测试了容器新增的逻辑，非常健壮

---

2025.01.10
用正则表达式完成了服务器ssh中容器读取到数据库的增加；
完成了界面重新架构，增加了呼吸样式 ；
添加了用户列表页面、容器列表页面和服务器列表页面；
测试了服务器多开docker脚本，再ssh添加到系统的应用。
